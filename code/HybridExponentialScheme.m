function [Xpaths,Upaths,dW] = HybridExponentialScheme(N,n,T,x0,b,sigma,gamm,c,kappa,varargin)
...
if ~isempty(T) && ~isempty(tX)
    error(['HybridExponentialScheme: You cannot use both the ''T'' and ''tX'' ',...
          'parameter at the same time.']);
elseif isempty(T) && isempty(tX)
    error(['HybridExponentialScheme: You must use either the ''T'' parameter or the ',...
          '''tX'' parameter.']);    
end

if ~isempty(tU) && ~isempty(T)
    error(['HybridExponentialScheme: You cannot use both the ''T'' and ''tU'' ',...
          'parameter at the same time.']);    
end

if ~isempty(tU) && ~returnU
    error(['HybridExponentialScheme: If the ''tU'' parameter is used',...
          ' the ''returnU'' parameter must be set to true.']);
end

if returnU && isempty(T) && isempty(tU)
    error(['HybridExponentialScheme: If the returnU = true then either ',...
...
if ~isempty(tX)
    ...
    if size(tX,1) > 1
        error('HybridExponentialScheme: The ''tX'' parameter must be a row vector.');
    end
end
if ~isempty(tU)
    if any(diff(tU) <= 0)
        error(['HybridExponentialScheme: The values of the ''tU'' vector must be strictly',...
               'increasing.']);
    end    
    if tU(1) < 0
        error('HybridExponentialScheme: Negative time points are not allowed.');
    ...
    if size(tU,1) > 1
        error('HybridExponentialScheme: The ''tU'' parameter must be a row vector.');
    end    
end
...
elseif size(b,1) == 1 && size(b,2) == 1
   ...
   b_is_constant = true;
    ...
    error('HybridExponentialScheme: ''b'' parameter is not valid.');
end
...
elseif size(sigma,1) == 1 && size(sigma,2) == 1
   ...
   sigma_is_constant = true;
    ...
    error('HybridExponentialScheme: ''sigma'' parameter is not valid.');
end
...
if ~isempty(Z) && ~isempty(W_bold)
    error(['HybridExponentialScheme: You cannot specify both the ''Z'' and ''W_bold'' ',...
           'parameter at the same time.']);
end

% Check kappa value is ok:
if mod(kappa,1) ~= 0
    error('HybridExponentialScheme: Kappa must be integer valued.');
end
if kappa < 0 || kappa > floor_nT
    error('HybridExponentialScheme: Kappa must be an integer between 0 and floor(n*T).');
end
...
if isempty(W_bold)
    ...
    if isempty(Z)
        ...
        if size(Z,1) ~= (M-1)*N || size(Z,2) ~= kappa + 1
            error(['HybridExponentialScheme: The Z-matrix containing the i.i.d. standard ',...
    ...
    if size(W_bold,1) ~= N || size(W_bold,2) ~= M - 1 || size(W_bold,3) ~= kappa + 1
        error('HybridExponentialScheme: The pre-simulated W_bold-matrix has the wrong dimensions.');
    end
...
if kappa > 0
    if isempty(w)
        ...
        if size(w,1) ~= kappa || size(w,2) ~= 1
            error('HybridExponentialScheme: The w-vector must be of the size [kappa x 1].');
        end
