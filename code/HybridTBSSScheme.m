...
%
%   conv_method:  [1x1 string or empty] Options are 'fft' and 'conv2'. If 'fft' is chosen we use 
%                 the fast Fourier transform to compute the convolution. If 'conv2' is chosen 
%                 we instead use Matlab's conv2 function. If left empty or unspecified 
%                 (recommended) we use 'conv2' when the vectors to be convolved are smaller than 
%                 800 in length and otherwise use 'fft'. This was found experimentally to be the 
%                 optimal choice (in terms of speed).
%
